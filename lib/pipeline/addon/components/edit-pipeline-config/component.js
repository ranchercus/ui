...
export default Component.extend({
  ...
  actions:              {
    save(success){
      ...
      if ( current !== origin ) {
        ...
        const envSelector = (get(this, 'envSelector') || []);
        const env = {};
    ...
    selectTemplate() {
      get(this, 'modalService').toggleModal('modal-pipeline-template', {
        save: (config) => {
          const key = get(this, 'selectedBranch');
          const branch = (get(this, 'pipeline.branches') || []).findBy('branch', key);
          ...
          set(this, 'selectedConfig', config);
          set(branch, 'config', config);
        },
      });
    },
  },
  ...
  branchDidChange: observer('pipeline.branches.@each.config', 'selectedBranch', function(){
    ...
    set(this, 'envSelector', get(branch, 'config.env'));
  }),
